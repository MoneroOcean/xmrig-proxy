...
public:
    ...
    inline uint64_t statsDiff() const       { return (result.assignedDiff ? result.assignedDiff : (miner() && miner()->customDiff() ? std::min(miner()->customDiff(), result.diff) : result.diff)); }
